...
%      'numPointsEllipse' - The number of points for drawing the ellipse,
%      default is 100 (JLBC, 27-JUNE-2006)
%
...
NUM_POINTS_ELLIPSE = prop.numPointsEllipse / 2;
...
if r==3 & c==3
  ...
  [x,y,z] = getpoints(Cxy,prop.clip,NUM_POINTS_ELLIPSE);
 % h1=plot3(x0+k*x,y0+k*y,z0+k*z,prop.style);hold on
  [y,z,x] = getpoints(Cyz,prop.clip,NUM_POINTS_ELLIPSE);
 % h2=plot3(x0+k*x,y0+k*y,z0+k*z,prop.style);hold on
  [z,x,y] = getpoints(Czx,prop.clip,NUM_POINTS_ELLIPSE);
...
%   x and y.
function [x,y,z] = getpoints(C,clipping_radius,NUM_POINTS_ELLIPSE)

n=NUM_POINTS_ELLIPSE; % Number of points around ellipse
