...
close all
cameraCalibrator;
imL = imread('IMG10.jpg');
if(size(imL,3)==3); imL=rgb2gray(imL); end;
imR = imread('IMG5.jpg');
if(size(imR,3)==3); imR=rgb2gray(imR); end;
scale=720/size(imL,1);
...
di = h1(1,:)-h2(1,:);
xli=h2(1,:); yli=h2(2,:);
kx=cameraParams.IntrinsicMatrix(1,1);
ky=cameraParams.IntrinsicMatrix(2,2);


Xi = b * (xli - cx) ./ di;
Yi = b * (-(kx/ky)*(yli - cy)) ./ di;
Zi = f * b ./ di;
figure
plot3(Xi,Yi,Zi,'.');